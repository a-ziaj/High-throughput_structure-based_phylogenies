...
tmscore_matrix_file="${matrix_dir}/tmscore_matrix_ID$job_id.tsv"
...
for ((i=0; i < num_files; i++)); do
    ...
    for ((j=0; j < num_files; j++)); do
        if [ $i -eq $j ]; then
            ...
            echo -ne "\t${tmscore_matrix["$f2,$f1"]}" >> "$tmscore_matrix_file"
            continue
        fi
